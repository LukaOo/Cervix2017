...
do
    ...
    function Hdf5Provider:__init(config, batchSize)  
        ...
        self.classes_table = {}
        for i=1, #self.classes_labels do
           self.classes_table[i] = self:getFilesList(self.InputPath .. self.classes_labels[i])
        end
        ...
        k = 1
        for c = 1, #self.classes_labels do
           for i = 1, #self.classes_labels do
              self.list[k] = {[1] = k}
              k = k + 1
           end
    ...
    function Hdf5Provider:preprocess()
        if self.ds_name == 'train' then
              return transform.Compose{
                     transform.ColorNormalize(transform.meanstd),
                    }
        else
              return transform.Compose{
                     transform.ColorNormalize(transform.meanstd),
    ...
    function Hdf5Provider:__sub_indices(indices, permutate)
      local _input   = {}
      local _target  = {}
      local permutate = permutate or false
      local v = self.indices[indices]
      for im_idx =1, v:size(1) do
        -- read input and target
        target, k = unpack(self.list[v[im_idx]])
        
        local input =  self:get_tensors(self:get_paths(self.class_table[target][k], target))
    ...
    function Hdf5Provider:sub(start, stop)
        ...
        if start <= stop then    
            ...
            indices = torch.linspace(start, stop, start-stop+1)
    ...
    function Hdf5Provider:get_tensors(path_input, target)
          ...
          -- get images
          local input  = self.preprocessor( h5_file:read( fld ):all():clone():double() )
          input = resize_tensor(input)
          
          local target = torch.LongTensor({1,1}):fill(target)
