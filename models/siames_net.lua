...
function LoadPretrainedNet()
    ...
    net:replace(function(module)
       if torch.typename(module) == 'nn.Linear' then
         ...
         return nn.Linear(module.weight:size(2), module.weight:size(2), false)
       else
         return module
       end
     end)
...
function CreateResNet(cinput_planes)
   ...
   local cnn = nn.Sequential():add( LoadPretrainedNet() )
   local cnn_l =  cnn
   local cnn_r =  cnn:clone('weight','bias', 'gradWeight','gradBias','running_mean','running_std', 'running_var')
   pt:add(cnn_l):add(cnn_r)
   net:add(pt)
   if opt.criterion == 'Dual' then
     local ctab = nn.ConcatTable()
     ctab:add(nn.Sequential():add(nn.BatchNormalization(embeding_size)):add(nn.ReLU(true)):add(nn.Linear(embeding_size, net_config.class_count )))
     ...
     local distance_branch = nn.Sequential():add(nn.NarrowTable(2,2)):add(nn.PairwiseDistance(2)) -- pairmise distance
     ctab:add(classifier_branch):add(distance_branch)
     net:add(ctab):add(nn:FlattenTable()) -- output is table classifier and pairwice distance for HingeEmbeddingCriterion
   else
